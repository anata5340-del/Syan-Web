...
export default function AddQuiz({
  ...
  const handleOk = async () => {
    ...
    formEntries.forEach(([key, value]) => {
      ...
      formData.append(`quiz[${key}]`, `${updatedValue}`);
    });
    ...
    if (mode === "edit") {
      try {
        await axios.patch(`/api/quizes/${data._id}`, formData);
        refetch();
    ...
    try {
      await axios.post("/api/quizes", formData);
      refetch();
  ...
  function goToQuizDetailPage() {
    router.push("/admin/quiz/123");
  }
