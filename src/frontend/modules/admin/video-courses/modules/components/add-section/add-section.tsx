...
export const AddSection = ({
  ...
  const handleOk = async () => {
    ...
    formEntries.forEach(([key, value]) => {
      ...
      formData.append(`section[${key}]`, `${updatedValue}`);
    });

    if (image) formData.append("image", image);

    if (!videoCourseId || !moduleId) return;
    ...
    if (mode === "edit") {
      ...
      try {
        ...
        refetch();
        toast.success("Video Course Module Section Updated Successfully");
      } catch (error) {
        console.error(error);
        toast.error("Video Course Module Section Updation Failed");
      } finally {
        toggleModal();
        setImage(undefined);
      }
    ...
    try {
      await axios.post(
        `/api/videoCourses/${videoCourseId}/modules/${moduleId}/section`,
        formData
      );
      refetch();
      handleCancel();
      toast.success("Video Course Module Section Added Successfully");
    } catch (error) {
      console.error(error);
      toast.error("Video Course Module Section Addition Failed");
    }
  };
