...
export const AddSubSection = ({
  ...
  const handleOk = async () => {
    ...
    formEntries.forEach(([key, value]) => {
      ...
      formData.append(`subSection[${key}]`, `${updatedValue}`);
    });

    if (image) formData.append("image", image);

    if (!videoCourseId || !moduleId || !sectionId) return;
    ...
    if (mode === "edit") {
      ...
      try {
        ...
        handleCancel();
        onAdd();
        toast.success("Video Course Module Sub Section Updated Successfully");
      } catch (error) {
        console.error(error);
        toast.error("Video Course Module Sub Section Updation Failed");
      }
      return;
    }
    ...
    try {
      await axios.post(
        `/api/videoCourses/${videoCourseId}/modules/${moduleId}/section/${sectionId}/subSection`,
        formData
      );
      refetch();
      handleCancel();
      onAdd();
      toast.success("Video Course Module Sub Section Added Successfully");
    } catch (error) {
      console.error(error);
      toast.error("Video Course Module Sub Section Addition Failed");
    } finally {
      setImage(undefined);
    }
  };
